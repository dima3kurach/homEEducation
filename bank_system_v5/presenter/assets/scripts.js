...
const inputFilter = input => {
   switch (input.name) {
      ...
      case 'passIDtoFindClient': inputManager(input, 'freeStr', 'passID'); break
      case 'passIDtoAddCard': inputManager(input, 'freeStr', 'passID'); break
      case 'summMinus': inputManager(input, 'number'); break
      case 'summPlus': inputManager(input, 'number'); break
...
const errSwitcher = inputCell => {
   const label = inputCell.input.closest('label')
